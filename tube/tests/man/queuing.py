...
class MQMixin(object):
    ...
    def test_append(self):
        self.assertEqual(
            self.qclass().append('foo').balanceout().end(), 'foo'
        )

    def test_prepend(self):
        self.assertEqual(
            self.qclass().prepend('foo').balanceout().end(), 'foo'
        )
    ...
    def test_undo(self):
        ...
        queue.append(1).append(2).undo(2).balanceout()
        self.assertListEqual(queue.snapshot(), [6, 5, 4, 3, 2, 1, 1, 2, 3, 1])
    ...
    def test_insync(self):
        ...
        self.assertEqual(list(q.inflow), list(q.outflow))
